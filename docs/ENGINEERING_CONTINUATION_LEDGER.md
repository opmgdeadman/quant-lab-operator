...
2. immediately after refresh, reload startup authority and this ECL, verify production remains aligned at release SHA `10d3b18d7f2a0019ba9944849c2d318c00c104ac`, then register `btc-regime-momentum-24-168-v1` before observing any result with exact immutable preregistration `{template:regime_momentum, feature_set_id:close-regime-momentum-v1, parameters:{lookback:24, regime_lookback:168, threshold_percent:0.5}}` and the unchanged dataset, walk-forward, cost, judge, and evidence-integrity policies; after registration, explicitly advance lifecycle from `proposed` to `admitted` before evaluation; never infer, skip, or auto-apply this state transition;
