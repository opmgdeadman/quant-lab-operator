...
Make Stage 14 hypotheses deterministically executable and independently judgeable before admitting the first production hypothesis:

1. replace execution-critical freeform preregistration fields with a bounded typed research specification that declares an allowed strategy family/template, feature inputs, parameter bounds, dataset identity, immutable walk-forward partition policy, cost model, evidence-integrity checks, and judge policy in machine-verifiable form; preserve human-readable economic mechanism and failure-mode text separately;
2. because the production Stage 14 registry is still empty, migrate the contract now rather than create compatibility debt; reject unknown templates, undeclared features, out-of-bound parameters, changed preregistration hashes, post-result rescue fields, and any specification that cannot be reproduced from completed BTC-USD 1h candles;
3. implement a deterministic Stage 14 evaluation adapter that converts only admitted typed hypotheses into immutable historical evidence using the existing next-completed-candle, fee/slippage/carry, no-look-ahead, walk-forward, and evidence-integrity primitives; caller-supplied performance metrics are prohibited;
4. implement an independent Stage 14 judge that consumes only sealed evaluator artifacts and append-only forward evidence, records immutable verdicts/reason codes, and may unlock the hypothesis lifecycle `qualified` transition only for judge-qualified evidence; qualification still grants no Stage 13 production promotion authority, no paper-main activity, and no live-capital authority;
5. add adversarial regression coverage for post-result mutation, fabricated metrics, unsupported templates/features, judge bypass, stale evidence, lineage recycling, and qualification without sealed independent evidence; then validate exact SHA, migrate, deploy Main and Recovery, commission the zero-state judge/evaluator surfaces, and only after those controls pass admit the first economically grounded Stage 14 production hypothesis.

Do not register a production hypothesis until this executable preregistration and independent-judge boundary is deployed and verified. Stage 13 directional institutional selection remains the sole production promotion authority; its judge, qualified-only selection, paper-main safe-idle behavior, and live-capital qualification gates remain unchanged.
