...
Continue Stage 14 with one materially distinct preregistered hypothesis rather than tuning the rejected EMA idea:

1. reload Startup Authority and this Git ECL, verify `btc-ema-24-96-trend-v1` remains terminal `rejected`, and preserve its rejection memory unchanged;
2. register `btc-donchian-72-breakout-v1` as an operator-origin BTC-USD `1h` breakout hypothesis using template `donchian_breakout`, feature set `ohlc-donchian-v1`, parameters `{lookback:72}`, dataset `btc-usd-1h-completed-4320-v1`, and the unchanged institutional walk-forward, cost, independent-judge, and evidence-integrity policies; the economic test is whether a three-day channel breakout captures sustained repricing with materially lower switching frequency than the rejected EMA crossover;
3. advance from `proposed` to `admitted`, run the deterministic sealed historical evaluation, advance to `testing`, and run the independent judge without changing the preregistration after observing results;
4. accept the verdict unchanged: historical failure becomes terminal `rejected`; historical pass with insufficient forward evidence remains `testing`; only an independent judge-qualified verdict may unlock `qualified`;
5. record the cycle evidence and next highest-information-gain research action in this Git ECL while preserving Stage 13 as sole production promotion authority, paper-main safe-idle behavior, and disabled live capital absent the existing evidence gate plus explicit owner approval.
