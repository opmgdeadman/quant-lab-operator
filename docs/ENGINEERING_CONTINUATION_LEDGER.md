...
Resume immediately after one ChatGPT Quant Lab connector refresh and complete the first governed Stage 14 production-hypothesis cycle:

1. load Startup Authority and this Git ECL, then verify the refreshed `register_institutional_hypothesis` tool no longer requires irrelevant template parameters; for `ema_trend`, the valid parameter payload must contain exactly `fast` and `slow`;
2. verify repository/production alignment and the empty Stage 14 registry, then register `btc-ema-24-96-trend-v1` exactly as preregistered: operator-origin `trend`, BTC-USD `1h`, template `ema_trend`, feature set `close-ema-v1`, parameters `{fast:24, slow:96}`, dataset `btc-usd-1h-completed-4320-v1`, and the unchanged institutional walk-forward, cost, independent-judge, and evidence-integrity policies;
3. advance the hypothesis from `proposed` to `admitted`, run the deterministic sealed historical evaluation, then advance to `testing` and run the independent judge; do not change parameters, datasets, costs, partitions, judge thresholds, or failure criteria after observing results;
4. accept the independent verdict unchanged: if historical gates fail, transition to `rejected` with the verdict evidence; if historical gates pass but forward evidence is insufficient, leave the hypothesis in `testing` and establish the exact append-only forward-evidence next action; only a judge-qualified verdict may unlock `qualified`;
5. record the completed cycle and exact evidence in this Git ECL, preserve Stage 13 as the sole production promotion authority, preserve paper-main safe-idle behavior, and keep live capital disabled absent the existing evidence gate plus explicit owner approval.

The connector refresh is the only current owner boundary. No production hypothesis was registered before it.
