...
Operate Stage 14 as a continuous research conveyor while untouched forward evidence accrues. Waiting for evidence is a passive clock, never the institution's primary work:

1. preserve the untouched evidence clock. `btc-donchian-72-breakout-v1` still requires genuine post-testing forward rows, and the missed 22:05 UTC opportunity may never be backfilled. Before adding more hypotheses that depend on the new collector, prove its first genuine append-only production row; if that row is absent or invalid, treat the collector path as P0 and repair it without manufacturing evidence;
2. keep an active research clock running in parallel on every productive operator shift. Use available execution budget for source-grounded hypothesis discovery, economic-mechanism analysis, regime/feature research, preregistration design, historical hostile testing, rejection-memory analysis, and only the minimum engineering needed to unlock materially distinct hypothesis classes. Status-only waiting is prohibited;
3. stop treating parameter grids as hypothesis discovery. The current rolling factory's eight-candidate batch contains only EMA-cross and RSI-mean-reversion variants and produced zero qualified candidates, seven rejections, and one insufficient-evidence result. Future research must prioritize materially different mechanisms rather than nearby parameter values of failed families;
4. prioritize the next research frontier around regime-conditioned trend/momentum, volatility state, volume/liquidity context, and price-discovery or cross-market features when trustworthy data is available. Every candidate must still have an explicit economic premise, immutable preregistration, realistic costs, next-completed-candle execution, and unchanged independent judging;
5. once the forward collector proves one genuine row, replace serial one-hypothesis waiting with bounded parallelism: historically test candidates continuously, allow only historical survivors to consume forward-testing slots, and maintain up to four materially distinct Stage 14 hypotheses in forward testing at once. A candidate waiting on its 168-hour evidence minimum must never block discovery or historical rejection of the next candidate;
6. optimize for durable risk-adjusted edge and a higher quality-adjusted hypothesis hit rate, not raw trade win rate, experiment count, or activity. No result-dependent tuning, threshold rescue, same-candle execution, synthetic evidence, or repeated testing of a rejected idea without materially new evidence;
7. keep `btc-donchian-72-breakout-v1` in `testing` until the unchanged judge has at least 168 genuine post-testing hourly cycles, at least three closed trades, positive cumulative forward return, maximum drawdown at or below 12%, and evidence integrity passed; do not spam judge verdicts before the minimum cycle gate can pass;
