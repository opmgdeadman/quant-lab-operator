...
Continue Stage 14 productively while untouched Donchian forward evidence accumulates autonomously:

1. on the first operator run after the 22:05 UTC eligible scheduler boundary, read `btc-donchian-72-breakout-v1` forward progress and require at least one immutable post-testing cycle with non-null isolated portfolio state; if the eligible cycle is absent, gapped, duplicated inconsistently, or integrity-failed, stop research expansion and open/repair one P0 hardening incident before any evidence can be backfilled;
2. keep `btc-donchian-72-breakout-v1` in `testing` until the unchanged judge has at least 168 genuine post-testing hourly cycles, at least three closed trades, positive cumulative forward return, maximum drawdown at or below 12%, and evidence integrity passed; do not repeatedly create judge verdicts while the minimum cycle gate is impossible to pass;
3. while that real-time evidence matures, allocate research capacity to one materially distinct third preregistered BTC-USD 1h hypothesis from a different economic family rather than tuning either prior result; source-validate the exact template parameter shape first, prefer a mean-reversion family to maximize information gain against the rejected EMA trend and historically passed Donchian breakout, and freeze all data, cost, walk-forward, judge, and failure criteria before evaluation;
4. run the third hypothesis through admission, sealed historical evaluation, testing, and the unchanged independent judge; preserve terminal rejection memory if it fails, or let it enter the same untouched forward-evidence path if historical gates pass;
