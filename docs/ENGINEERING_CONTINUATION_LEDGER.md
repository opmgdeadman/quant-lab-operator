...
Build, validate, deploy, and commission the missing append-only Stage 14 forward-evidence collector for historically passed `testing` hypotheses:

1. implement a bounded hourly collector that reuses the proven directional signal and signed 1.0x paper-rebalance math, maintains isolated per-hypothesis virtual portfolio state, and appends one immutable `institutional_research_forward_evidence` record per eligible completed execution candle;
2. prohibit retroactive evidence: a hypothesis may begin forward evidence only from the first signal candle that closes after it entered `testing`, with execution only at the next completed candle open; never backfill pre-testing market outcomes into forward evidence;
3. preserve the exact institutional fee, slippage, short-carry, next-completed-candle execution, gap-free-data, duplicate-protection, evidence-integrity, Stage 13 authority, paper-only, and live-capital-disabled boundaries; add any mutable forward portfolio projection in a new migration while keeping evidence rows immutable;
4. wire the collector into the hourly production path after market-data availability, add adversarial tests for no-look-ahead, no pre-testing contamination, idempotent replay, exposure cap, fee/carry accounting, state continuity, immutable evidence, and judge compatibility, then run CI, apply migration, deploy the exact SHA, and verify repository/production alignment;
5. commission the live collector. `btc-donchian-72-breakout-v1` must remain `testing` until at least 168 genuine post-testing hourly forward cycles, at least three closed trades, positive forward return, maximum drawdown at or below 12%, and forward evidence integrity all pass the unchanged independent judge. Do not synthesize, accelerate, backfill, or lower these gates.
