...
Prove the newly deployed Stage 14 forward-evidence collector on its first lawful live scheduler cycle before expanding the testing population:

1. do not register a fourth hypothesis before the first eligible Donchian forward cycle; the shared collector is a new P0 production dependency and must prove one genuine append-only cycle before additional `testing` hypotheses increase its blast radius;
2. on the first operator run at or after the permanent scheduler invocation following `2026-08-19T22:00:00.000Z` execution-candle close, read `btc-donchian-72-breakout-v1` and require `forward_evidence_count >= 1`, non-null isolated forward portfolio state, post-testing timing, contiguous candle lineage, evidence integrity passed, and no inconsistent duplicate; the expected first scheduler opportunity is 22:05 UTC;
3. if that evidence is absent or invalid, immediately stop research expansion, create one P0 hardening incident, diagnose the scheduled collector path, repair/test/deploy it, and never backfill the missed outcome; if it is valid, record the commissioning proof and resume the hypothesis program with one fourth materially distinct preregistered family selected for highest information gain;
4. keep `btc-donchian-72-breakout-v1` in `testing` until the unchanged judge has at least 168 genuine post-testing hourly cycles, at least three closed trades, positive cumulative forward return, maximum drawdown at or below 12%, and evidence integrity passed; do not repeatedly create judge verdicts before the minimum cycle gate can pass;
5. keep Stage 13 as the sole canonical production promotion authority, preserve paper-main safe idle and reconciled accounting, and keep live capital disabled absent the existing evidence gate plus explicit owner approval.
