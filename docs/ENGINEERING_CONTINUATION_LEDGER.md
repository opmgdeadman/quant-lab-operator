...
4. only after explicit `admitted` state is proven, run the sealed 4,320-candle five-window evaluation and unchanged independent judge exactly once on the frozen spec. If history fails, advance terminal `rejected` with no rescue tuning. If every historical gate passes, advance to `testing` and let it occupy the second of at most four parallel forward slots beside Donchian;
