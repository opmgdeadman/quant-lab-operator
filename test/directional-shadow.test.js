...
test("position-hold mode preserves same signed quantity without a recurring fill", () => {
  const opened = applySignedRebalance({
    portfolio: portfolio(),
    targetExposure: 1,
    executionPrice: 100,
  ...
  const held = applySignedRebalance({
    portfolio: heldPortfolio,
    targetExposure: 1,
    executionPrice: 110,
    markPrice: 115,
    hoursElapsed: 1,
    holdSameDirection: true,
  });
  assert.equal(held.position_quantity, opened.position_quantity);
  assert.equal(held.quantity_delta, 0);
  assert.equal(held.fee, 0);
  assert.equal(held.status, "held");
  assert.ok(held.equity > opened.equity);
});

test("position-hold mode leaves entry exit reversal and v1 default economics unchanged", () => {
  const base = portfolio();
  assert.deepEqual(
    applySignedRebalance({ portfolio: base, targetExposure: 1, executionPrice: 100, markPrice: 101 }),
    applySignedRebalance({ portfolio: base, targetExposure: 1, executionPrice: 100, markPrice: 101, holdSameDirection: false }),
  );
  const longPortfolio = portfolio({ cash_balance: 10000, position_quantity: 50, average_entry: 100, equity: 10000, peak_equity: 10000, gross_exposure_multiple: 0.5 });
  for (const targetExposure of [0, -1]) {
    assert.deepEqual(
      applySignedRebalance({ portfolio: longPortfolio, targetExposure, executionPrice: 110, markPrice: 109 }),
      applySignedRebalance({ portfolio: longPortfolio, targetExposure, executionPrice: 110, markPrice: 109, holdSameDirection: true }),
    );
  }
});

test("position-hold short marks still pay carry without same-direction fees", () => {
  const opened = applySignedRebalance({ portfolio: portfolio(), targetExposure: -1, executionPrice: 100, markPrice: 100 });
  ...
  const held = applySignedRebalance({ portfolio: shortPortfolio, targetExposure: -1, executionPrice: 99, markPrice: 95, holdSameDirection: true });
  assert.equal(held.quantity_delta, 0);
  assert.equal(held.fee, 0);
  assert.ok(held.carry > 0);
  assert.equal(held.position_quantity, opened.position_quantity);
});
