    ...
    <main class="content" id="overview">
      ...
      <section class="section" id="operations">
        ...
        <article class="panel table-panel"><div class="panel-head"><div class="panel-title"><b>Forward-operation evidence</b><span>No qualified champion creates a durable idle cycle instead of a fallback trade</span></div>${badge(forward?.state || "none")}</div><div class="list">
          ${listRow("cycle", "Latest cycle", forward?.cycle_id || "No cycle recorded")}${listRow("clock", "Expected completed candle", date(forward?.expected_closed_at))}${listRow("alert", "Execution blockers", reasons(forward?.blocker_codes))}
        </div></article>
      </section>
      ...
      <section class="section" id="research">
        ...
        <article class="panel table-panel"><div class="panel-head"><div class="panel-title"><b>Selection state</b><span>Fallback selection is prohibited</span></div>${badge(championSelection?.state || "none")}</div><div class="list">${listRow("crown", "Champion", `<span data-live="champion">${esc(champion || "None — safe idle")}</span>`)}${listRow("check", "Qualified candidates", integer(championSelection?.eligible_count))}${listRow("alert", "Selection blockers", reasons(championSelection?.blocker_codes))}</div></article>
      </section>
...
function listRow(iconName,title,content){return `<div class="list-row"><div class="list-icon">${icon(iconName)}</div><div><b>${esc(title)}</b><span>${content}</span></div></div>`}
