...
- The production MCP now supports the new `regime_momentum` template, but the current ChatGPT connector still advertises the pre-release registration schema. One Quant Lab connector refresh is therefore the only current external boundary before the preregistered regime-momentum hypothesis can be admitted through ChatGPT.

## Current Action

Convert the deployed regime-momentum capability into the first parallel Stage 14 research cycle, then keep the conveyor moving while forward clocks accrue:

1. the only immediate external boundary is one ChatGPT Quant Lab connector refresh so the public registration schema exposes production template `regime_momentum` and feature set `close-regime-momentum-v1`; no additional code or deployment is required before that refresh;
2. immediately after refresh, reload startup authority and this ECL, verify repository and production deployment SHAs are exactly aligned at the current validated release, then register `btc-regime-momentum-24-168-v1` before observing any result with exact immutable preregistration `{template:regime_momentum, feature_set_id:close-regime-momentum-v1, parameters:{lookback:24, regime_lookback:168, threshold_percent:0.5}}` and the unchanged dataset, walk-forward, cost, judge, and evidence-integrity policies; after registration, explicitly advance lifecycle from `proposed` to `admitted` before evaluation; never infer, skip, or auto-apply this state transition;
3. the hypothesis premise is state-dependent momentum persistence: positive shorter-horizon momentum may have higher information value when the broader 168-hour return regime is already positive, while a long/flat structure avoids assuming a symmetric short edge. Predeclare failure modes including regime reversal, late/crowded entries, cost drag, whipsaw near the regime boundary, and underparticipation in fast recoveries;
4. only after explicit `admitted` state is proven, run the sealed 4,320-candle five-window evaluation exactly once on the frozen spec; after the sealed evaluation exists, explicitly advance lifecycle from `admitted` to `testing` before invoking the unchanged independent judge, then run that judge exactly once. If the judge rejects historical evidence, advance terminal `rejected` with no rescue tuning. If historical gates pass but forward evidence is incomplete, remain `testing` and occupy the second of at most four parallel forward slots beside Donchian. Only the unchanged judge may support later `qualified` state after both historical and forward gates pass;
5. after that disposition, continue source-grounded discovery rather than waiting: next priority is a materially distinct volatility-state-conditioned mechanism using only trustworthy already-normalized data unless a new data capability is independently justified. Historical losers are killed quickly; historical survivors consume forward slots; evidence clocks never block new research;
6. keep `btc-donchian-72-breakout-v1` in `testing` until the unchanged judge has at least 168 genuine post-testing hourly cycles, at least three closed trades, positive cumulative forward return, maximum drawdown at or below 12%, and evidence integrity passed. Its first genuine row is commissioning proof, not performance proof;
7. optimize for durable risk-adjusted edge and quality-adjusted hypothesis hit rate, never raw experiment count or cosmetic win rate. No result-dependent tuning, threshold rescue, same-candle execution, synthetic evidence, or backfill of missed forward outcomes;
8. keep Stage 13 as the sole canonical production promotion authority, preserve paper-main safe idle and reconciled accounting, and keep live capital disabled absent the existing evidence gate plus explicit owner approval.
